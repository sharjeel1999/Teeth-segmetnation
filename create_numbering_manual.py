...
data_path = 'C:\\Users\\Sharjeel\\Desktop\\codes\\teeth_numbered_data\\Missed_again.npy'
data = np.load(data_path, allow_pickle=True)

print('data shape: ', data.shape)

manually_numbered_samples = []
manually_missed_samples = []
...
for sample in data:
    ...
    if len(un) == 33:
        ...
        min1 = input('give min value for first')
        max1 = input('give max value for first')
        min2 = input('give min value for second')
        max2 = input('give max value for second')
        ...
        for j in range(256, 512):
            for i in range(int(min2), int(max2)):
                combined_copy = np.copy(conc_part)
                pix = combined_copy[i, j, :]
    ...
    cc = input('is it correct')
    if cc == '1':
        print('Added to manually numbered')
        manually_numbered_samples.append([image, mask, instance, numbered_mask, weights, 1])
    else:
        print('added to missed')
        manually_missed_samples.append([image, mask, instance, numbered_mask, weights, 1])
    

manually_numbered_samples = np.array(manually_numbered_samples)
manually_missed_samples = np.array(manually_missed_samples)
print('Manually numbered samples: ', len(manually_numbered_samples))
print('Manually missed samples: ', len(manually_missed_samples))

#np.save('correct_numbered_samples.npy', numbered_data)