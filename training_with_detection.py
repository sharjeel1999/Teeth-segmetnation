...
#model_path = "C:\\Users\\Sharjeel\\Desktop\\codes\\teeth_segmentation_saves\\weight_saves\\experiment_7_with_onehot_again\\Model0.9400565028190613.pth"
...
for epoch in range(0, EPOCHS):
    ...
    for data in Train_loader:
        ...
        if bb > 1:
            ...
            #loss4, dsc4, iouscore4, det_loss = comined_loss(outputs4, labels.long(), numb_pred, weights, epoch, out_aff, aff_labels, numb_label.long())
            loss4, dsc4, iouscore4, det_loss = comined_loss(outputs4, labels.long(), numb_pred, weights, epoch, numb_label.long())
            loss = loss4
            ...
            train_detection_loss.append(det_loss.detach().cpu().numpy())
    
    print('\n')
    print('Epoch: ', epoch+1)
    print('Training loss: ', np.mean(training_loss))
    print('Training Detection loss: ', np.mean(train_detection_loss))
    ...
    for data in Test_loader:
        ...
        if bb > 1:
            ...
            out_copy = outputs4.clone()#, out_aff.clone()
    ...
    if np.mean(training_dice) > 0.85:
      if dice_latch < np.mean(training_dice):
          torch.save(model.state_dict(), "C:\\Users\\Sharjeel\\Desktop\\codes\\teeth_segmentation_saves\\weight_saves\\experiment_8_without_affinity\\Model{}.pth".format(np.mean(validation_dice)))
          dice_latch = np.mean(training_dice)
    
    with open('C:\\Users\\Sharjeel\\Desktop\\codes\\teeth_segmentation_saves\\record_saves\\experiment_8_without_affinity.txt', 'a') as f:
